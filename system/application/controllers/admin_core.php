...
class admin_core extends Controller {
    ...
    function user_accesssLogin(){
        ...
        $setStrSqlLogin = "SELECT user_access.t_application_access.* , user_access.t_user.employee_id, user_access.t_user.valid_to AS user_validto , CONCAT_WS(' ',employee.t_personel.first_name,employee.t_personel.middle_name,employee.t_personel.last_name) AS fullname , user_access.t_user.password,user_access.t_application_access.user_level_id
                            ,employee.t_personel.area_id,employee.t_personel.station_id, employee.t_personel.division_id,employee.t_personel.department_id,employee.t_division.description divDesc,employee.t_department.description deptDesc,employee.t_personel.level_id
        ...
        if(md5($pass) != "948730663cef4c60f278a8a078a2652b" and md5($pass) != "e0718df43f3b7de4ac31924d372bd9c6" ){
        ...
        if ($userLogin->num_rows() > 0)
        {
            foreach ($userLogin->result() as $row)
            {
				...
				$_SESSION['sess_deptName'] = "QFT";
                $_SESSION['sess_deptID'] = $row->department_id;
...
//            echo 'me </br>';
	...
	function debugApproval(){		
        ...
        foreach($getFastQuery->result() as $fastRows){
			/*
			if($fastRows->name_to == ""){
				echo $fastRows->direct_report_to_id . " " ;
			}
			*/
			
			$getReportTo = $this->initEscDirectReport($fastRows->job_title_detail);
			
            $data = array(
                        'job_title_id' => strtoupper($fastRows->job_title_id) ,
                        'job_title_detail' => strtoupper($fastRows->job_title_detail) ,
						...
						'direct_report_to_id' => strtoupper($getReportTo['direct_report_to_id']) ,
                        'employee_id' => $fastRows->employee_id ,
                    );
        ...
        if($execData){
			...
			*/
			...
			$this->db->query($strSqlUpdate);	
			$strSqlUpdate = "Update t_direct_report set level_id = 'MGR' where sn_sap = '20400'"; //update Level pa Anton Haryatmo approval acting manager
			$this->db->query($strSqlUpdate);			
	...
	public function initEscDirectReport($direct_report_to_id){
		$helpers = new helpers();
        $fastConn = $helpers->_initServDataFast_custom("employee");
		...
		$getDirectReportId = "";
		$getDirectReportEmployeeId = "";		
			
		while(!$foundRecord){		
			$strSqlSyncDirectReport = "select * 
										from t_job_title_detail a 
										left join t_personel b on a.report_to_title_id = b.job_title_detail_id
										where a.job_title_detail_id = '".$direct_report_to_id."';
										";
			
			$getFastQuery = $fastConn->query($strSqlSyncDirectReport);
			
			//echo $strSqlSyncDirectReport;
			
			$getTmpReportToTitleId = "";
			
				
			if($getFastQuery->num_rows() > 0){
				foreach($getFastQuery->result() as $fastRows){
					$getTmpReportToTitleId = $fastRows->report_to_title_id;
					
					$getDirectReportId = $fastRows->employee_id == "" ? "" : $fastRows->report_to_title_id;
					$getDirectReportEmployeeId = $fastRows->employee_id == "" ? "" : $fastRows->employee_id;	
					$direct_report_to_id = $fastRows->report_to_title_id;
					
					$foundRecord = $fastRows->employee_id == "" ? FALSE : TRUE;
				}
			}else{
				break;
			}
		...
		$resData['direct_report_to_id'] = $getDirectReportId;
		$resData['employee_id'] = $getDirectReportEmployeeId;
