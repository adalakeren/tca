...
class fc_core extends Controller {
    ...
    function initHistoryTable($getStart, $getEnd, $getCondition, $getType, $getDept, $getStat) {
		...
		// echo "<pre>"; print_r($arrUserArea); echo "</pre>"; exit();
		$arrUserNew = implode ('","', $arrUserArea);
        $arrUserNew;
        //========= 
        ...
        */
        ...
        $randIds = rand('1234567890', 6);
        $randId = $randIds;

        // echo $randId;exit;

        //$this->db->('create table if not exists tca_fc_'.$randId.' AS (SELECT id_ca_trans, ca_guid, date_request, requester_name, requester_level_detail, requester_sn, requester_userid, requester_station, requester_dept, requester_div, request_type, request_amount, settled_amount, last_appv, payment_apv, appv_flow, appv_flow_status, appv_flow_settle, appv_flow_settle_status, appv_complete, appv_settle_complete, purpose, fin_settle, fin_settle_user, fin_settle_datetime, fin_settle_rejreason, settle_mail_counter, input_user, input_datetime, input_user_settle, input_datetime_settle, has_toplevel_esc, has_toplevel_esc_settle, isdelete, final_settledate, sap_ready, sap_sent FROM t_ca_trans WHERE ca_guid <> "" and isdelete = 0)');
        // $this->db->where_in("requester_userid", $arrUserArea);
        // $getQuery = $this->db->get();

		/* $tempQuery = 'CREATE TABLE tca_fc_'.$randId.' AS SELECT id_ca_trans, ca_guid, date_request, requester_name, requester_level_detail, requester_sn, requester_userid,
					  requester_station, requester_dept, requester_div, request_type, request_amount, settled_amount, last_appv, payment_apv, appv_flow, appv_flow_status,
					  appv_flow_settle, appv_flow_settle_status, appv_complete, appv_settle_complete, purpose, fin_settle, fin_settle_user, fin_settle_datetime, fin_settle_rejreason,
					  settle_mail_counter, upload_f1, input_user, input_datetime, input_user_settle, input_datetime_settle, has_toplevel_esc, has_toplevel_esc_settle, isdelete,
					  final_settledate, sap_ready, sap_sent FROM t_ca_trans WHERE ca_guid <> "" AND isdelete = 0 AND requester_userid IN ("'.$arrUserNew.'") AND YEAR(input_datetime) >= 2018'; */
		$tempQuery = 'CREATE TABLE tca_fc_'.$randId.' AS SELECT id_ca_trans, ca_guid, date_request, requester_name, requester_level_detail, requester_sn, requester_userid,
					  requester_station, requester_dept, requester_div, request_type, request_amount, settled_amount, last_appv, payment_apv, appv_flow, appv_flow_status,
					  appv_flow_settle, appv_flow_settle_status, appv_complete, appv_settle_complete, purpose, fin_settle, fin_settle_user, fin_settle_datetime, fin_settle_rejreason,
					  settle_mail_counter, upload_f1, input_user, input_datetime, input_user_settle, input_datetime_settle, has_toplevel_esc, has_toplevel_esc_settle, isdelete,
					  final_settledate, sap_ready, sap_sent FROM t_ca_trans WHERE ca_guid <> "" AND isdelete = 0 AND YEAR(input_datetime) >= 2018';
		
		
		$execTemp = $this->db->query($tempQuery);
		// echo $tempQuery;

		$queryIndex = "CREATE INDEX id_ca_trans_idx ON tca_fc_".$randId."(id_ca_trans)";
		$exeIndex = $this->db->query($queryIndex);
		$queryAlter = "ALTER TABLE tca_fc_".$randId." ENGINE = InnoDB";
		$exeAlter = $this->db->query($queryAlter);
    ...
    (CASE WHEN d.isSuj = 0 THEN d.destination WHEN d.isSuj = 1 THEN e.fleet_schedule WHEN ISNULL(d.isSuj) THEN d.destination END) AS RealDestination, (CASE WHEN a.appv_flow_settle LIKE "%END%" THEN 1 ELSE 0 END) AS SettleStatus,', FALSE)
        ...
        $queryDel = "DROP TABLE IF EXISTS tca_fc_".$randId;
        $exeDel = $this->db->query($queryDel);
        ...
        $strTblRes .= '<th style="width:50px">' . 'User Finance Complete' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:50px">' . 'Date Time Finance Complete' . $tmplTbl['heading_cell_end'];
        ...
        if ($getQuery->num_rows() == 0) {
            ...
            foreach ($getQuery->result() as $rows) {
                ...
                $strTblRes .= '<td style="text-align:center;">' . $rows->fin_settle_user . $tmplTbl['cell_end'];
                $strTblRes .= '<td style="text-align:center;">' . $rows->fin_settle_datetime . $tmplTbl['cell_end'];
...
//        echo $this->db->last_query();
        ...
        $tableData = '<table>
                        <tr>
                            ...
                            <td style="background:gray" >Finance Settle User</td>                            
                            <td style="background:gray" >Finance Settle Date Time</td>                            
                        </tr>
                        ';
        ...
        foreach ($getQuery->result() as $rowS) {
            ...
            */
            $tableData .= '<tr>         
                                ...
                                <td>'.$rowS->purpose.'</td>
                                <td>'.$getStatus.'</td>
                                <td>'.$rowS->fin_settle_user.'</td>
                                <td>'.$rowS->fin_settle_datetime.'</td>
                            </tr>';         
