...
class ca_upload extends Controller { 
	...
	function upload_request(){
		...
		$helpers = new helpers();
		$data['cmbCashType'] = $helpers->initCmbAllValue_cond2("cmbCashType", 
			"t_jenis_transaksi_19022018", "desc_trans", "id_trans", "",$this->session->userdata('userEmpID'));
	...
	function IsSuj($tcano)
	{
        ...
        $query = "SELECT b.isSuj
			FROM t_ca_trans_19022018 a
			INNER JOIN tt_suj_transaction_19022018 b ON a.ca_guid = b.ca_guid
	...
	function IsManifestExist($tcano)
	{
        ...
        $query = "SELECT no_manifest FROM tr_manifest_19022018 WHERE id_ca_trans = '".$tcano."'";
	...
	function GetTransApproveStatus($ca_guid)
	{
		...
		$query = $this->db->query("SELECT apprv_id FROM tr_trans_approve_status_19022018 WHERE ca_guid = '".$ca_guid."' ");//AND approver='".$getUserJobDetail."'");
	...
	function GetMainTransaction($tca_id)
	{
		$this->load->database();
		$this->db->select('*');
		$this->db->from('t_ca_trans_19022018');
    	$this->db->where('CONCAT(id_ca_trans, "-TCA-", MONTH(date_request), YEAR(date_request)) = ', $tca_id );
	...
	function do_upload_request(){
		...
		$helpers = new helpers();
		$datas['cmbCashType'] = $helpers->initCmbAllValue_cond2("cmbCashType", 
			"t_jenis_transaksi_19022018", "desc_trans", "id_trans", "",$this->session->userdata('userEmpID'));
		
		$suj_not_founded = array();
	    $suj_error_list = array();
	    print_r($this->upload->do_upload('userfile'));die();
	        ...
	        for ($i = 2; $i <= $sheets['numRows']; $i++)
      		{
		        if (isset($sheets['cells'][$i]))
		        {
		        	...
		        	// insert to t_ca_trans_19022018
		        	$tblName = "t_ca_trans_19022018";
		        	$data = array(
		        		'ca_guid' => $setGuid,
		        	...
		        	if ($execData) {
		        		...
		        		if ($isSUJ)
		        		{
		        			$tblName = "tr_trans_approve_status_19022018";
		                    $data = array(
		                        'ca_guid' => $setGuid,
	       			 ...
	       			 if ($execData) {
			                ...
			                $execData = $this->db->insert('tt_suj_transaction_19022018', $data);
	       			 	}
	       			 	else if ($sujno == "0")
	       			 	{
	...
	function exportError($suj_error_list)
	{
		...
		function do_upload_settle() {
			...
			if($this->upload->do_upload('userfile')) {
				...
				if($dataExcel > 10){
					echo '<script>alert("Maximum allowed data is 10!");</script>';			
				}
				else{
					$datas["error"] = "Upload Success";
					for($i = 2; $i <= $sheets['numRows']; $i++){
		         		if(isset($sheets['cells'][$i])){
							...
							$t_ca_trans_19022018 = $this->GetMainTransaction($tcano);
							$isSUJ = $this->IsSuj($tcano);
							$getReqType = $t_ca_trans_19022018->request_type;
							$getAmount = $t_ca_trans_19022018->request_amount;

							$getLastApprovalMsgDetail  = $ajaxManager->initLastApproval($getReqType,$getAmount,0, "CONTROLLER");
							$getLastApprovalMsgDetail2  = $ajaxManager->initUserApproval($getUserSessionLevelDetail, 
							 	$getLastApprovalMsgDetail['hidApprove'], 0, "CONTROLLER");
						    $getApprovalFlow = substr($getLastApprovalMsgDetail2['hidApvFlow'], 0,-1);
				         	$getFirstApprover = explode(",", $getApprovalFlow);

				        	//if($this->IsValidTCA($tcano))
							//{
						
							//$settled_amount = $this->GetSettleAmount($tcano);
							$settled_amount = $isSUJ ? $t_ca_trans_19022018->request_amount : $sheets['cells'][$i][2];
						
							//if ((int)$t_ca_trans_19022018->request_amount < (int)$sheets['cells'][$i][2])
							//{
							//	$appv_complete = 10;
							//}
							//else if ((int)$t_ca_trans_19022018->request_amount > (int)$sheets['cells'][$i][2])
							//{
							///	$appv_complete = 11;
							//}
							//else 
			        		...
			        		$this->db->where('CONCAT(id_ca_trans, "-TCA-", MONTH(date_request), YEAR(date_request)) = ',$sheets['cells'][$i][1]);
						 	...
						 	if(!$execData) {
					        	...
					        	//$t_ca_trans_19022018 = $this->GetMainTransaction($sheets['cells'][$i][1]);
								
					        	//insert to manifest

					         	if(!$this->IsManifestExist($tcano)){
					         		$tblName = "tr_manifest_19022018";
						         	$data = array(
									         		'id_ca_trans' => $t_ca_trans_19022018->id_ca_trans,
									         		'ca_guid' => $t_ca_trans_19022018->ca_guid,
				         		...
				         		if($execData){
					         		...
					         		if($isSUJ) {
						 	        	...
						 	        	$this->db->where('ca_guid',$t_ca_trans_19022018->ca_guid);
						 				$execData = $this->db->update($tblName, $data);
						 				...
						 				if(!$execData) {
					 		        		...
					 		        		$transapproveStatus = $this->GetTransApproveStatus($t_ca_trans_19022018->ca_guid);
	...
	function getDirectReportToID($employeeid)
	{
		...
		$sql = "SELECT * FROM t_direct_report_19022018 WHERE employee_id = '" . $employeeid . "'";
	...
	function getDirectReport($job_title)
	{
		...
		$sql = "SELECT * FROM t_direct_report_19022018 WHERE job_title_detail = '" . $job_title . "'";
