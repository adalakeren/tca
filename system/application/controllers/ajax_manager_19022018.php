...
class ajax_manager extends Controller {
    ...
    function getDirectReportToId($employeeId){
        ...
        $getReportToQuery = $this->db->select('a.direct_report_to_id')
                        ->from('t_direct_report_19022018 a')                        
                        ->where($whereArry)->get();
        // print_r($this->db);die();
    ...
    function initUserApprovalSub($getCurrLevID, $getLastApproval, $getCurrLevelSeq) {
        ...
        $getReportToQuery = $this->db->select('a.*, b.name nameto,c.otor_seq')
                        ->from('t_direct_report_19022018 a')
                        ->join('t_direct_report_19022018 b', 'a.direct_report_to_id = b.job_title_detail', 'left')
    ...
    function approval() {
        ...
        switch ($getType) {
            case "approve":
                ...
                $execData = $this->db->update('tr_trans_approve_status_19022018', $data);
                ...
                $this->db->select('a.apprv_id, b.*')
                        ->from('tr_trans_approve_status_19022018 a')
                        ->join('t_ca_trans_19022018 b', 'a.ca_guid = b.ca_guid', 'left')
                        ->where($whereArry)
...
//                    $this->sendEmailRemiderApproval($getEmailTo, $getGuid, 'APV');
                ...
                $execData = $this->db->update('t_ca_trans_19022018', $data);
                //==================
                //send email
                if (count($getAppFlowArray) != count($getAppFlowStatusArray)) {
                    $this->sendEmailToNextAppv($getAppFlowArray[count($getAppFlowStatusArray)], $getGuid, "STD");
            ...
            case "reject":
                ...
                if ($execData) {
                    //here get GUID first
                    $whereArry = array('a.apprv_id' => $getId, 'a.approve_type' => 'STD');
                    $getQueryGuid = $this->db->select('a.*')->from('tr_trans_approve_status_19022018 a')->where($whereArry)->get();
            ...
            case "finsetreject":
                ...
                $getQueryGuid = $this->db->select('a.*')->from('t_ca_trans_19022018 a')->where($whereArry)->get();
                foreach ($getQueryGuid->result() as $rows) {
                    $getGuid = $rows->ca_guid;
                }
                ...
                $execDelData = $this->db->delete('tr_trans_approve_status_19022018');
                //
    ...
    function sendEmailRemiderApproval($getEmailTo, $getCaGuid, $getType) {
        ...
        $this->db->select('a.*, b.desc_trans, MONTH(a.date_request) as bulan , YEAR(a.date_request) as tahun')
                ...
                ->join('t_jenis_transaksi_19022018 b', 'a.request_type = b.id_trans', 'left')
                ->where($whereArry);
        $getQuery = $this->db->get();
...
//        $strUpdateCounter = "UPDATE t_ca_trans_19022018 SET settle_mail_counter = settle_mail_counter+1 WHERE id_ca_trans=".$getCaId;
...
//                    $this->sendEmailRemiderSettle2($getEmailTo, $getGuid,'APV');
    ...
    function initCAView() {
        ...
        $this->db->select('a.*, b.desc_trans, MONTH(a.date_request) as bulan , YEAR(a.date_request) as tahun, c.isSuj ')
                ->from('t_ca_trans_19022018 a')
                ->join('t_jenis_transaksi_19022018 b', 'a.request_type = b.id_trans', 'left')
                //aditional by ALfan
                ->join('tt_suj_transaction_19022018 c', 'a.ca_guid = c.ca_guid', 'left')
                //======================================
                ->where($whereArry)
                ->order_by('input_datetime', 'desc');
...
//            $res .= '</tr>';
    ...
    function getUserDetailLevel($getUserLevelDetail) {
        $res = '';
        $getQuery = $this->db->select('*')->from('t_direct_report_19022018 a')->where('a.job_title_detail', $getUserLevelDetail)->get();
        foreach ($getQuery->result() as $getRow) {
            $res = $getRow->name;
        }
        return $res;
...
//        echo $setBody;
        ...
        $helpers->sendMail($getEmailTo, $setBody);
        $queryGetMailCounter = "SELECT settle_mail_counter from t_ca_trans_19022018 where".$whereQuery;
        $query = $this->db->query($queryGetMailCounter);

        $strUpdateCounter = "UPDATE t_ca_trans_19022018 SET settle_mail_counter = ".$query->first_row()->settle_mail_counter."+1 WHERE".$whereQuery;
        $execData = $this->db->query($strUpdateCounter);
