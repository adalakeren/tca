...
class ca_upload extends Controller {
    ...
    function upload_exim() {
        $data["header"] = "Upload Export & Import";
        $data["error"] = "";

        $this->load->view('ca_header');
        $this->load->view('ca_bread', $data);
        $this->load->view('upload_exim');
        $this->load->view('ca_footer');
    }

    function upload_settle_exim() {
        $data["header"] = "Upload Settle Exim";
        $data["error"] = "";

        $this->load->view('ca_header');
        $this->load->view('ca_bread', $data);
        $this->load->view('upload_settle_exim');
        $this->load->view('ca_footer');
    }
   ...
   function do_upload_request() {
        ...
        if ($ext == "xlsx"){
            echo "<script>alert('Format excel harus .xls !');</script>";
            $datas["error"] = "Upload Failed";
            echo 5;
        }
        ...
        if ($this->upload->do_upload('userfile')) {
            ...
            for ($i = 2; $i <= $sheets['numRows']; $i++) {
                if (isset($sheets['cells'][$i])) {
                    ...
                    //here to get level id
                    $getDirectToIdByUserReqLvl = $ajaxManager->getDirectLevelId($getLastApproval);
                    $getApprovalLevelId = $getDirectToIdByUserReqLvl;


                    if($getApprovalLevelId == "CRD" || $getApprovalLevelId == "STF"){
                        if($isSUJ){
                            $getDirectToIdByUserReq2 = $ajaxManager->getDirectReportToId2($getLastApproval);
                            $getLastApproval = $getDirectToIdByUserReq2;
                        }
                    }

                    // echo $getDirectToIdByUserReq2;exit;

                    // if($isSUJ){
                    //     $getDirectToIdByUserReq2 = $ajaxManager->getDirectReportToId2($getLastApproval);
                    //     $getLastApproval = $getDirectToIdByUserReq2;
                    // }
                    // echo $getLastApproval;exit;
                    ...
                    // if($getLevelID == "STF"){
                    //     $getApprovalFlow = $getFirstApprover[1] . "," . $getFirstApprover[2];
                    // }
                    // echo $getApprovalFlow;exit;

                    // insert to t_ca_trans

                    // $bank_driverid = $sheets['cells'][$i][6];
                    // echo $bank_driverid;exit;
                    ...
                    if($getApprovalLevelId == "CRD" || $getApprovalLevelId == "STF" && $isSUJ){
                            $tblName = "t_ca_trans";
                            $data = array(
                                'ca_guid' => $setGuid,
                                ...
                                // 'payment_apv' => $isSUJ ? $getDirectToIdByUserReq : $getLastPaymentApproval,
                                ...
                                'payment_apv' => $isSUJ ? $getDirectToIdByUserReq2 : $getLastPaymentApproval,
                                'appv_flow' => $isSUJ ? $getDirectToIdByUserReq2 : $getApprovalFlow,
                                'appv_flow_status' => $isSUJ ? $getDirectToIdByUserReq2 : $getFirstApprover[0], //here new cash complete flow
                                'input_user' => $getUserSess,
                                'input_datetime' => date("Y-m-d H:i:s"),
                                // 'purpose' => isset($sheets['cells'][$i][8]) ? $sheets['cells'][$i][8] : ""
                                'purpose' => isset($sheets['cells'][$i][9]) ? $sheets['cells'][$i][9] : ""
                            );
                    } else{
                        $tblName = "t_ca_trans";
                        $data = array(
                    ...
                    //     'purpose' => isset($sheets['cells'][$i][8]) ? $sheets['cells'][$i][8] : ""
                    // );
                    // echo $data['appv_flow_status'];exit;
                    ...
                    if ($execData) {
                        ...
                        //         'approver' => $getDirectToIdByUserReq2,
                        //         'approve_type' => 'STD',
                        ...
                        if($getApprovalLevelId == "CRD" || $getApprovalLevelId == "STF" && $isSUJ){
                            ...
                            if($getRowUserAccess != 0 && $getReqType == 'trans14'){
                                ...
                                $execData = $this->db->insert('tr_trans_approve_status', $data);
                                ...
                                echo 'sudah update data di tr_trans_approve_status dan t_ca_trans';exit;
                            } else{
                                $tblName = "tr_trans_approve_status";
                                $data = array(
                                    'ca_guid' => $setGuid,
                    ...
                    if ($execData) {
                        ...
                        $data = array(
                            ...
                            'bank_drivername' => $sheets['cells'][$i][7],
                            'bank_driverrekno' => $sheets['cells'][$i][8],
                            'insert_by' => $getRequester,
                            'insert_ts' => date("Y-m-d H:i:s"),
                            'update_by' => $getRequester,
                            'update_ts' => date("Y-m-d H:i:s"),
    ...
    function exportError($suj_error_list) {
        ...
        $tableData = "<table><tr>
		                  <td>SUJ Code</td><td>Origin</td><td>Destination</td><td>Truck Type</td><td>Advance Amount</td><td>Id Driver</td><td>Nama Driver</td><td>No Rekening</td><td>Remark</td></tr>";
        // $tableData .= "<tr>";
        // $tableData .= "<td>SUJ Code</td>";
        // $tableData .= "<td>Origin</td>";
        // $tableData .= "<td>Destination</td>";
        // $tableData .= "<td>Truck Type</td>";
        // $tableData .= "<td>SUJ Code</td>";
        // $tableData .= "<td>SUJ Code</td>";
        // $tableData .= "<td>SUJ Code</td>";
        // $tableData .= "<td>SUJ Code</td>";
        ...
        for ($i = 0; $i < count($suj_error_list); $i++) {
            ...
            $column9 = isset($suj_error_list[$i][9]) ? $suj_error_list[$i][9] : "";
    ...
    function do_upload_exim() {
        ...
        $this->load->library('upload', $config);
        $this->load->library('Spreadsheet_Excel_Reader');
        $datas["header"] = "Upload Export & Import";
        $datas["error"] = "Upload Success";
        ...
        if ($this->upload->do_upload('userfileExim')) {
            $data = $this->upload->data();
            @chmod($data['full_path'], 0777);
            ...
            for ($i = 2; $i <= $sheets['numRows']; $i++) {
                if (isset($sheets['cells'][$i])) {
                    ...
                    $da_no = $sheets['cells'][$i][1];
                    $len_da_no = strlen($da_no);
                    if ($len_da_no == "12" && is_numeric($da_no)){
                        // echo "masuk kondisi";exit;
                    } else{
                        echo "<script>alert('Panjang DA Number ".$da_no." tidak boleh lebih dari 12 karakter!');</script>";
                        echo 5;
                    }

                    $remark_dn = $sheets['cells'][$i][5];
                    if ($remark_dn == "DN"){
                      $remark_dn_new = "DA MGF";
                    } elseif ($remark_dn == "Cost") {
                      $remark_dn_new = "DA Clearance";
                    } else {
                      $remark_dn_new = $sheets['cells'][$i][5];
                    }
                    ...
                    $data_excel = array(
                        ...
                        'input_datetime' => date("Y-m-d H:i:s"),
                        'da_no' => $da_no,
                        'aju_no' => $sheets['cells'][$i][2],
                        'exim_importir' => $sheets['cells'][$i][3],
                        'exim_ref_no' => $sheets['cells'][$i][4],
                        // 'remark_dn' => $data['values'][$i]['E'],
                        'exim_remark_dn' => $remark_dn_new,
                        'exim_cost_name' => $sheets['cells'][$i][6],
                        // 'exim_estimate_value' => $sheets['cells'][$i][7],
                        'request_amount' => $sheets['cells'][$i][7],
                        'exim_pic_transfer' => $sheets['cells'][$i][8],
                        'exim_bank_name' => $sheets['cells'][$i][9],
                        'exim_rek_no' => $sheets['cells'][$i][10]
                    );

                    // print_r($data_excel);
                    $execData = $this->db->insert('t_ca_trans', $data_excel);
    ...
    // additional by yudhis
    function do_upload_settle_exim() {
        $datas["header"] = "Upload Settle Exim";
        $datas["error"] = "";

        // ========================
        $currentDateTime = date('ymdHis');
        // $newFileName = pathinfo($_FILES['userfileExim']['name'], PATHINFO_FILENAME)."_".$currentDateTime.".".pathinfo($_FILES['userfileExim']['name'], PATHINFO_EXTENSION);
        $newFileName = pathinfo($_FILES['userfileExim']['name'], PATHINFO_FILENAME).".".pathinfo($_FILES['userfileExim']['name'], PATHINFO_EXTENSION);
        log_message('debug', 'gp>>> akan simpan file ' . $newFileName);
        ...
        $this->load->library('upload', $config);
        $this->upload->do_upload('userfileExim');
        // ========================


        // read data excel and save to database
        // $file = './_UploadedFiles/Bulk_Settlement_'. date("Ymd") .'.xls';
        $file = './upload/Bulk_Settlement_'. date("Ymd") .'.xls';
        ...
        for ($i = 2; $i <= $totalRow; $i++) {
            ...
            $tca_no = $data['values'][$i]['A'];
            $len_tca_no = strlen($tca_no);

            // get tca_number
            $tca_no = trim($tca_no);
            $tcanos = explode("-", $tca_no);
            $new_tca_no = $tcanos[0];
            // print_r($new_tca_no);exit;
            // ====================================================
            ...
            $data_excel = array(
              'request_amount' => $data['values'][$i]['B'],
              'settled_amount' => $data['values'][$i]['C']
            );

            $this->db->where('id_ca_trans = ', $new_tca_no);
            $execData = $this->db->update($tblName, $data_excel);
        ...
        if($this->db->affected_rows() > 0){
            echo 'the file has been updated to database.';
        } else{
            echo 'the file failed update to database!';
        }
    ...
    // additional by yudhis
    function do_upload_settle_exim_2() {
        $config = array(
            'upload_path' => FCPATH . 'upload/',
            'allowed_types' => 'xls',
        );
        ...
        if ($this->upload->do_upload('userfileExim')) {
            ...
            for ($i = 2; $i <= $sheets['numRows']; $i++) {
                if (isset($sheets['cells'][$i])) {
                    ...
                    // $tca_no = $sheets['cells'][$i][1];
                    // $requestAmount = $sheets['cells'][$i][2];
                    // $settleAmount = $sheets['cells'][$i][3];
                    $tca_no = $sheets['cells'][$i][3];
                    $requestAmount = $sheets['cells'][$i][12];
                    $settleAmount = $sheets['cells'][$i][16];

                    $tca_no = trim($tca_no);
                    $tcanos = explode("-", $tca_no);
                    $new_tca_no = $tcanos[0];

                    echo $settleAmount;exit;
...
// =======================================================================
    ...
    function do_upload_settle_exim_3() {
        log_message('debug', 'gp>>> mulai do_upload_settle');
        $currentDateTime = date('ymdHis');
        log_message('debug', 'gp>>> akan simpan file ' . $currentDateTime);
        $newFileName = pathinfo($_FILES['userfileExim']['name'], PATHINFO_FILENAME)."_".$currentDateTime.".".pathinfo($_FILES['userfileExim']['name'], PATHINFO_EXTENSION);
        ...
        log_message('debug', 'gp>>> akan simpan file' . implode(" ", $_FILES['userfileExim']));
        if ($this->upload->do_upload('userfileExim')) {
            ...
            if ($dataExcel > 10) {
                ...
                for ($i = 2; $i <= $sheets['numRows']; $i++) {
                    if (isset($sheets['cells'][$i])) {
                        ...
                        $requestAmount = $sheets['cells'][$i][12];
                        $settled_amount = $sheets['cells'][$i][16];
                        ...
                        if (!$execData) {
                            ...
                            if ($execData) {
                                ...
                                if ($isSUJ) {
                                    ...
                                    // echo 'masuk kondisi SUJ';exit;

                                    if($getRowUserAccess != 0){
                                        $data = array(
                                            'ca_guid' => $t_ca_trans->ca_guid,
                                            // 'approver' => $getDirectToIdByUserReq,
                                            'appr_status' => 1,
                                            'approve_type' => 'SET',
                                            'inputUser' => $getRequesterSn,
                                            'inputDateTime' => date("Y-m-d H:i:s")
                                        );
                                        $execData = $this->db->insert('tr_trans_approve_status', $data);

                                        if($execData){
                                            // echo 'masuk ke kondisi 2 non suj';exit;
                                            $data2 = array(
                                                'appv_complete' => 5,
                                                'appv_settle_complete' => 3,
                                                'appv_flow_settle_status' => $getApprovalFlow. ',END',
                                            );
                                            $this->db->where('ca_guid', $t_ca_trans->ca_guid);
                                            $execData = $this->db->update('t_ca_trans', $data2);
                                        }
                                    ...
                                    // echo 'masuk kondisi Non SUJ';exit;
                                    $appStep = explode(",", $getApprovalFlow);
                                    foreach ($appStep as $appv) {
                                        // $tblName = "tr_trans_approve_status";
                                        // $data = array(
                                        ...
                                        // $this->db->where('ca_guid', $t_ca_trans->ca_guid);
                                        // $this->db->where('approver', $appv);
                                        log_message('debug', 'gp>>> akan update ke tabel tr_trans_approve_status non suj');
                                        // $execData = $this->db->update($tblName, $data);

                                        if($getRequesterSn == 'bkr0016'){
                                            $data = array(
                                                'ca_guid' => $t_ca_trans->ca_guid,
                                                'approver' => $appv,
                                                'appr_status' => 1,
