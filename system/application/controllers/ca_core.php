...
//        $throwData['cmbCashType'] = $helpers->initCmbAllValue_cond("cmbCashType", "t_jenis_transaksi", "desc_trans", "id_trans", "");
    ...
    function saveNewCash() {
        ...
        $addType = "";
        if (isset($_POST['chkPLS'])){
          $addType = "PLS";
          $estimateDate = date("Y-m-d", strtotime($_POST['txtEstimateDatePLS']));
        }
        if (isset($_POST['chkNonPLS'])){
          $addType = "";
          $estimateDate = date("Y-m-d H:i:s");
        }
        if (isset($_POST['chkPP4L'])){
          $addType = "PP4";
          $estimateDate = date("Y-m-d", strtotime($_POST['txtEstimateDatePP4']));
        }
        if (isset($_POST['chkPP4K'])){
          $addType = "";
          $estimateDate = date("Y-m-d H:i:s");
        }

        //$estimateDate = date("Y-m-d", strtotime($_POST['txtEstimateDate']));
        // echo $estimateDate;exit();
    ...
    */
        ...
        // penambahan
        // cek user akses khusus
        $t_ca_user_access = $this->GetRequesterAccess($getUserID);
...
//            'appv_flow_settle' => $getApprovalFlow, //here settled cash complete flow
        ...
        if ($execData) {
          ...
          if($getRowUserAccess != 0 && $getReqType == 'trans14'){
            ...
            if($execData){
              $data2 = array(
                'appv_complete' => 7,
                'appv_flow_status' => $getApprovalFlow. ',END',
              );
              $this->db->where('ca_guid', $setGuid);
              $execData = $this->db->update('t_ca_trans', $data2);
            }
  ...
  //       }
    ...
    function saveSettlement($getId,$amount=null,$isRedirect=null) {
        ...
        //get manifest no
        $getManifestNo = $_POST['txtManifestNo'];
    ...
    function exportExcelFMode($getType, $getCondition) {
        ...
        $this->db->select("a.*, b.desc_trans, MONTH(a.date_request) as bulan , c.bank_driverid,
          ...
          e.suj_id,e.truck_type,e.truck_type,a.add_type,a.estimate_date")
                    ->from('t_ca_trans a')
                    ->join('t_jenis_transaksi b', 'a.request_type = b.id_trans', 'left')
        ...
        $getUserSess = $this->session->userdata('userName');
        $this->db->where(array('a.ca_guid <>' => "", 'a.isdelete' => 0, 'a.requester_userid' => $getUserSess, 'year(a.input_datetime)'=> 2019));
       ...
       // echo $this->db->last_query();exit;
        ...
        $tableData = '<table>
                <tr>
                  ...
                  <td style="background:gray" >Estimate Date</td>
                  <td style="background:gray" >Add Type</td>
                </tr>
        ';
        ...
        foreach ($getQuery->result() as $rowS) {
            ...
            if ($rowS->add_type == "REGULAR") {
                  $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '</span>';
                  $aType='';
                  $esDate='';
            } elseif ($rowS->add_type == "PLS") {
                  $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '-PLS' . '</span>';
                  $aType='PLS';
                  $esDate= $rowS->estimate_date;
            } elseif ($rowS->add_type == "PP4") {
                  $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '-PP4' . '</span>';
                  $aType='PP4';
                  $esDate= $rowS->estimate_date;
            } else {
              $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '</span>';
    ...
    function initHistTable($getStart, $getEnd, $getCondition, $getType, $getDept, $getStat) {
       ...
       //$getUserID = $this->session->userdata('userName');
        ...
        $whereArry = array('a.ca_guid <>' => "", 'a.isdelete' => 0, 'a.requester_userid' => $getUserSess);
    ...
    function cahistory() {
        ...
        if (isset($_POST['txtStartDate'])) {
            $getStartDate = $_POST['txtStartDate'];
        } else {
            $getStartDate = date('d-m-Y',strtotime("-1 days"));;
        }

        if (isset($_POST['txtEndDate'])) {
            $getEndDate = $_POST['txtEndDate'];
        } else {
            $getEndDate = date("d-m-Y");;
        }
        ...
        // $getStartDate = date("Y-m-d", strtotime($getStartDate));
        // $getEndDate = date("Y-m-d", strtotime($getEndDate));

        // $optradio = $this->input->post('optradio');
        // echo $optradio;
        if (isset($_POST['optradio'])) {
            $getOptradio = $_POST['optradio'];
        } else {
            if (isset($_GET['radio'])) {
                $getOptradio = $_GET['radio'];
            } else {
                $getOptradio = "";
            }
        }
        ...
        //pagination
        $getTable = $this->initHistoryTable($start, $end, $getStartDate, $getEndDate, $getOptradio, $getSearch);
        // $config['base_url'] = base_url() . 'index.php/ca_core/cahistory?paging=true&search=' . $getSearch;
        $config['base_url'] = base_url() . 'index.php/ca_core/cahistory?paging=true&search=' . $getSearch .'&radio=' . $getOptradio;
        $config['total_rows'] = $getTable['tblRes_numrow'];
        $config['per_page'] = '15';
        ...
        // $throwData['currDate'] = date("d-m-Y");
        // $throwData['currDateYes'] = date('d-m-Y',strtotime("-1 days"));

        $throwData['startDate'] = $getStartDate;
        $throwData['endDate'] = $getEndDate;

        $throwData['optRadio'] = $getOptradio;
    ...
    function initHistoryTable($getStart, $getEnd, $getStartDate, $getEndDate, $getOptradio, $getCondition) {
        ...
        if($getStartDate != "" && $getEndDate != "")
        {
          if($getOptradio == "1"){
            ...
            $whereArry = array('a.input_user' => $getUserSess, 'a.isdelete' => 0, 'a.input_datetime >=' => $getStartDate, 'a.input_datetime <=' => $getEndDate );
          } else{
        ...
        */
        ...
        $setSearchField = "concat_ws(' ',LOWER(requester_name),LOWER(e.bank_drivername),LOWER(a.purpose),concat(LOWER(a.id_ca_trans),'-TCA-',LOWER(MONTH(a.date_request)),LOWER(YEAR(a.date_request))))";
        ...
        $this->db->select('a.*, b.desc_trans, MONTH(a.date_request) as bulan , YEAR(a.date_request) as tahun,(CASE WHEN c.isSuj = 0 THEN "NON SUJ" WHEN c.isSuj = 1 THEN "SUJ" WHEN ISNULL(c.isSuj) THEN "-" END) AS IsSuj, d.expired as ExpiredDate,(case WHEN (c.id_cost_type_rate = 0 and c.isSuj = 0) THEN "No" WHEN (c.isSuj = 0 and c.id_cost_type_rate >= 0) THEN "Yes" WHEN c.isSuj = 1 THEN "No" end ) as IsExpired')
                ...
                ->join('tr_driver_account e', 'a.ca_guid = e.ca_guid', 'left')
                ->where($whereArry);
        ...
        */
        ...
        $strTblRes .= '<th style="width:160px" colspan="1">' . 'DA/Manifest No' . $tmplTbl['heading_cell_end'];
        ...
        if ($getQuery->num_rows() == 0) {
            ...
            foreach ($getQuery->result() as $rows) {
                ...
                if ($rows->appv_complete == "1") {
					...
					//$editDeleteButton = '';
					if ($rows->request_type == "trans14") $editDeleteButton = "";
                } elseif ($rows->appv_complete == "2") {
                    $getStatus = "<span class=\"redSpan\">TCA - Rejected</span>";
                    $editDeleteButton = '<input title="Rejected" type="image" src="' . base_url() . 'includes/icons/block.png" />';
                } elseif ($rows->appv_complete == "3") {
                ...
                $strTblRes .= '<td style="text-align:center;">' . $rows->man_no . $tmplTbl['cell_end'];
    ...
    function ca_eximhistory() {
        ...
        //pagination
        $getTable = $this->initEximHistoryTable($start, $end, $getSearch);
        $config['base_url'] = base_url() . 'index.php/ca_core/ca_eximhistory?paging=true&search=' . $getSearch;
        $config['total_rows'] = $getTable['tblRes_numrow'];
        $config['per_page'] = '15';
        ...
        $throwData['header'] = "<img src=\"" . base_url() . "includes/icons/zones.png\" alt=\"\"/> Cash Advance Exim History";
        $throwData['txtSearch'] = $getSearch;
        // $getTable = $this->initHistoryTable(0, 10, "");
        // $throwData['tblDatas'] = $getTable['tblRes'];
        $this->load->view('ca_exim_history', $throwData);
    }


    function initEximHistoryTable($getStart, $getEnd, $getCondition) {
        ...
        */
        ...
        // $strTblRes .= '<th style="width:100px" colspan="1">' . 'Expired Date' . $tmplTbl['heading_cell_end'];
        // $strTblRes .= '<th style="width:100px" colspan="1">' . 'Is Expired' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'DA No' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'AJU No' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'Importir' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'Ref No' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'Remarks' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'Cost Name' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'PIC Transfer' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="width:100px" colspan="1">' . 'No Rek.' . $tmplTbl['heading_cell_end'];
        ...
        if ($getQuery->num_rows() == 0) {
            ...
            foreach ($getQuery->result() as $rows) {
                ...
                $strTblRes .= '<td style="text-align:center;">' . $rows->exim_importir . $tmplTbl['cell_end'];
                $strTblRes .= '<td style="text-align:center;">' . $rows->exim_ref_no . $tmplTbl['cell_end'];
                $strTblRes .= '<td style="text-align:center;">' . $rows->exim_remark_dn . $tmplTbl['cell_end'];
                $strTblRes .= '<td style="text-align:center;">' . $rows->exim_cost_name . $tmplTbl['cell_end'];
                $strTblRes .= '<td style="text-align:center;">' . $rows->exim_pic_transfer . $tmplTbl['cell_end'];
                $strTblRes .= '<td style="text-align:center;">' . $rows->exim_rek_no . $tmplTbl['cell_end'];
...
//        header('Content-Disposition: attachment;filename="trainReport_'.$getStart.'_'.$getEnd.'.xls"');
    ...
    function exportExcel2($getOptRadio, $getStartDate, $getEndDate, $getCondition) {
        
        $getUserSess = $this->session->userdata('userName');

        // echo $getOptRadio;exit();
        ...
        $strTblRes .= '<th style="background:gray; color:white">' . 'DA/Manifest No' . $tmplTbl['heading_cell_end'];
        ...
        $strTblRes .= '<th style="background:gray; color:white">' . 'Estimate Date' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="background:gray; color:white">' . 'Add Type' . $tmplTbl['heading_cell_end'];
        $strTblRes .= $tmplTbl['row_end'];
        ...
        foreach ($getQuery->result() as $rowS) {
            ...
            if ($rowS->add_type == "REGULAR") {
                  ...
                  $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '-PLS' . '</span>';
                } elseif ($rowS->add_type == "DILUAR KOTA") {
                  $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '-PP4' . '</span>';
                } else {
                  $setTCAno = '<span style="font-weight:bold" >' . $rowS->id_ca_trans . '-TCA-' . $rowS->bulan . $rowS->tahun . '</span>';
                }
            ...
            $strTblRes .= '<td>' . $rowS->man_no . $tmplTbl['cell_end'];
            ...
            $strTblRes .= '<td>' . $rowS->estimate_date . $tmplTbl['cell_end'];
            $strTblRes .= '<td>' . $rowS->add_type . $tmplTbl['cell_end'];
            $strTblRes .= $tmplTbl['row_end'];           
...
//        echo $this->db->last_query();
        ...
        $strTblRes .= '<th style="background:gray; color:white">' . 'AJU No' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="background:gray; color:white">' . 'Importir' . $tmplTbl['heading_cell_end'];
        $strTblRes .= '<th style="background:gray; color:white">' . 'Ref No' . $tmplTbl['heading_cell_end'];
        ...
        foreach ($getQuery->result() as $rowS) {
            ...
            $strTblRes .= '<td>' . $rowS->exim_importir . $tmplTbl['cell_end'];
            $strTblRes .= '<td>' . $rowS->exim_ref_no . $tmplTbl['cell_end'];
            $strTblRes .= '<td>' . $rowS->exim_remark_dn . $tmplTbl['cell_end'];
            $strTblRes .= '<td>' . $rowS->exim_cost_name . $tmplTbl['cell_end'];
            $strTblRes .= $tmplTbl['row_end'];           
        ...
        $filename = date("Ymd")."_tca_exim.xls";
        header("Content-type: application/xls");
        header("Content-Disposition: attachment; filename=$filename");
